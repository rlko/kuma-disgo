...
var (
	...
	statusViewType     string = "detailed"
)
...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to set global commands: %v", err)
	}

	// Start the update loop
	go updateStatusLoop(client, kumaClient, cfg)
...
func updateStatusLoop(client bot.Client, kumaClient *kuma.Client, cfg *config.Config) {
	...
	for range ticker.C {
		...
		_, err := client.Rest().UpdateMessage(statusChannelID, statusMessageID, discord.NewMessageUpdateBuilder().
			SetEmbeds(*embed).
			Build(),
		)
		if err != nil {
